...
# The `when` fixture is not implemented yet (WIP). These tests are kept as a spec for
# that future feature and are out of scope for AppDaemon 4.5 compatibility.
pytestmark = pytest.mark.skip(reason="`when` fixture not implemented yet")
...
class LivingRoomMotionLight(hass.Hass):
    ...
    def light_on(self, entity: str, attribute: str, old: str, new: str, kwargs: dict) -> None:
        """Turn light on."""
        # pylint: disable=too-many-arguments
        # ^^ method signature inherited from appdaemon
        self.turn_on("light_switch")

    def light_off(self, entity: str, attribute: str, old: str, new: str, kwargs: dict) -> None:
        # pylint: disable=too-many-arguments
        # ^^ method signature inherited from appdaemon
        """Turn light off."""
...
def test_when_no_entity(living_room_motion_light, given_that, when, assert_that):
...
def test_when_no_specific_state(living_room_motion_light, given_that, when, assert_that):
...
def test_when_several_callbacks(living_room_motion_light, given_that, when, assert_that):
