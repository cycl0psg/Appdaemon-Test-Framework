...
class Test_get_now:
    ...
    async def test_get_now_returns_proper_time(self, scheduler):
        ...
        assert await scheduler.get_now() == pytz.utc.localize(datetime.datetime(2000, 1, 1, 0, 0))

    @pytest.mark.asyncio
    async def test_get_now_ts_returns_proper_time_stamp(self,  scheduler):
        # the time should default to Jan 1st, 2000 12:00AM
        assert await scheduler.get_now_ts() == pytz.utc.localize(datetime.datetime(2000, 1, 1, 0, 0)).timestamp()
...
class Test_time_movement:
    ...
    async def test_cant_set_start_time_with_pending_callbacks(self, scheduler):
        scheduled_time = scheduler.get_now_sync() + datetime.timedelta(seconds=10)
    ...
    @pytest.mark.asyncio
    async def test_fast_forward_to_time_in_future_goes_to_correct_time(self, scheduler):
        scheduler.sim_set_start_time(datetime.datetime(2015, 1, 1, 12, 0))
        scheduler.sim_fast_forward(datetime.time(14, 0))
        assert await scheduler.get_now() == pytz.utc.localize(datetime.datetime(2015, 1, 1, 14, 0))

    @pytest.mark.asyncio
    async def test_fast_forward_to_time_in_past_wraps_to_correct_time(self, scheduler):
        scheduler.sim_set_start_time(datetime.datetime(2015, 1, 1, 12, 0))
        scheduler.sim_fast_forward(datetime.time(7, 0))
        assert await scheduler.get_now() == pytz.utc.localize(datetime.datetime(2015, 1, 2, 7, 0))

    @pytest.mark.asyncio
    async def test_fast_forward_to_datetime_goes_to_correct_time(self, scheduler):
    ...
    @pytest.mark.asyncio
    async def test_fast_forward_by_timedelta_goes_to_correct_time(self, scheduler):
        scheduler.sim_set_start_time(datetime.datetime(2015, 1, 1, 12, 0))
        scheduler.sim_fast_forward(datetime.timedelta(days=1))
        assert await scheduler.get_now() == pytz.utc.localize(datetime.datetime(2015, 1, 2, 12, 0))
