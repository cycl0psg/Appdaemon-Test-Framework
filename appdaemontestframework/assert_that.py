...
class Was(ABC):
    ...
    @abstractmethod
    def set_to_option(self, newvalue, **kwargs):
        pass
...
class WasWrapper(Was):
    ...
    def set_to_option(self, newvalue, **service_specific_parameters):
        ...
        service_not_called = _capture_assert_failure_exception(
            lambda: self.hass_functions["call_service"].assert_any_call(
                ServiceOnAnyDomain("select_option"), **{"entity_id": entity_id, "option": newvalue, **service_specific_parameters}
            )
        )

        set_option_helper_not_called = _capture_assert_failure_exception(
            lambda: self.hass_functions["select_option"].assert_any_call(entity_id, newvalue, **service_specific_parameters)
        )
...
class WasNotWrapper(Was):
    ...
    def set_to_option(self, newvalue, **service_specific_parameters):
        ...
        val = _capture_assert_failure_exception(lambda: self.was_wrapper.set_to_option(newvalue, **service_specific_parameters))
        if not val:
            raise AssertionError("Should NOT have the value: " + str(self.was_wrapper.thing_to_check))
